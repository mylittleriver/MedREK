...
class RECIPE(BaseEditor):
    ...
    ############################# RECIPE Edit Related ############################
    ############################################################################
    def retrieve_ratio_and_get_ids_sim(self, input_queries: List[str]):
        # 获取 query 表征
        query_reps = self.knowl_rep_model(input_queries, knowl_or_query='q')  # [n, d]

        # sim_matrx: [n_query, n_knowledge+1]（含 prototype）
        sim_matrx = (query_reps @ self.knowledge_base.T) / self.cfg.knowledge_rep_dim**0.5

        # 原型相似度：第 0 列
        sim_with_prototype = sim_matrx[:, :1]  # [n, 1]

        # 对所有知识（含原型）排序，降序排列
        sorted_sim, order = torch.sort(sim_matrx, dim=1, descending=True)  # [n, edit_n+1]

        # 只取前 top-k，排除相似度低 & 不如 prototype 的
        mask = sorted_sim[:, :self.retr_top_k] > self.retr_min_sim
        mask &= sorted_sim[:, :self.retr_top_k] > sim_with_prototype

        # 获取满足条件的知识索引（按 query 分组）
        retrieved_ids = torch.masked_select(order[:, :self.retr_top_k], mask)
        retrieved_ids = torch.split(retrieved_ids, mask.sum(1).tolist())  # List[Tensor], len=n

        # === 相似度比值 ===
        n = sim_matrx.size(0)
        proto_sim = sim_with_prototype.squeeze(1)  # [n]

        # top-1 / prototype
        top1_sim = sorted_sim[:, 0]
        ratio_top1_vs_proto = top1_sim / (proto_sim + 1e-8)

        # self_sim 位置是 i+1（因为知识在 knowledge_base 中排在第 1~10 行）
        self_indices = torch.arange(n, device=sim_matrx.device)
        self_sim = sim_matrx[self_indices, self_indices + 1]
        ratio_self_vs_proto = self_sim / (proto_sim + 1e-8)

        return retrieved_ids, ratio_top1_vs_proto, ratio_self_vs_proto

    def retrieve_and_get_ids_sim(self, input_queries:List[str], post=False, type_metrics=None):
