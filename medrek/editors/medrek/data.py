...
def stack_xym(tokenizer, x_list, y_list, m_list, device):
    ...
    x = pad_sequence(x_list, True, tokenizer.pad_token_id)
    y = pad_sequence(y_list, True, tokenizer.pad_token_id)
    m = pad_sequence(m_list, True, 0)
    return x, y, m

class MEDREKTrainData():
    def __init__(self, tokenizer:AutoTokenizer, data_n = None, data_name = 'medcf', 
        ...
        if data_name.lower() in ['cf', 'counterfact']:
        ...
        elif data_name.lower() in ['medcf']:
            if data_path == None:
                data_path = os.path.join(ROOT_PATH, 'data/meta-train/medcf/train_single.json')
            self.sample_count, self.get_data_by_ids = self.__medcf__(tokenizer, data_n, data_path)
        elif data_name.lower() in ['medmcqa']:
            if data_path == None:
                data_path = os.path.join(ROOT_PATH, 'data/medmcqa/train.json')
            self.sample_count, self.get_data_by_ids = self.__medmcqa__(tokenizer, data_n, data_path)
        elif data_name.lower() in ['medfe']:
            if data_path == None:
                data_path = os.path.join(ROOT_PATH, 'data/meta-train/medfe/train.json')
            self.sample_count, self.get_data_by_ids = self.__medfe__(tokenizer, data_n, data_path)
            ...
            medcf_sample_count, medcf_get_data_by_ids = self.__medcf__(tokenizer, data_n, data_path['medcf'])
            ...
            def mix_get_data_by_ids(ids:List[int]):
                ...
                sample_count = [cf_sample_count, medcf_sample_count, zsre_sample_count, ripe_sample_count][r]
                ids = self.rng.integers(0, sample_count, len(ids)) 
                return [cf_get_data_by_ids, zsre_get_data_by_ids, ripe_get_data_by_ids][r](ids)
            self.sample_count, self.get_data_by_ids = cf_sample_count+medcf_sample_count+zsre_sample_count+ripe_sample_count, mix_get_data_by_ids
        else:
            raise KeyError
    ...
    # MedCF
    def __medcf__(self, tokenizer: AutoTokenizer, data_n, data_path):
        with open(data_path, 'r') as f:
            data = json.load(f)
        
        sample_count = min(len(data), data_n) if data_n is not None else len(data)
        
        relia_pt, gen_pt, loc_pt = [], [], []
        relia_xym, gen_xym, loc_xym = [], [], []
        
        progress_bar = tqdm(data[:sample_count], desc='MedCF data preparing...', dynamic_ncols=True)
        for d in progress_bar:
            # Reliability task
            relia_pt.append((d['prompt'], d['target_new']))
            relia_xym.append(pt2xym(tokenizer, d['prompt'], d['target_new']))
    
            # Generation task (rephrase)
            gen_pt.append({
                'rephrase': [(d['rephrase_prompt'], d['target_new'])]
            })
            gen_xym.append({
                'rephrase': [pt2xym(tokenizer, d['rephrase_prompt'], d['target_new'])]
            })
    
            # Locality task
            loc_pt.append({
                'original': [(d['locality_target_prompt'], d['locality_target_ground_truth'])],
                'mapping': [(d['locality_mapping_prompt'], d['locality_mapping_ground_truth'])],
                'struc': [(d['locality_struc_prompt'], d['locality_struc_ground_truth'])],
                'tokenSem': [(d['locality_tokenSem_prompt'], d['locality_tokenSem_ground_truth'])],
            })
            loc_xym.append({
                'original': [pt2xym(tokenizer, d['locality_target_prompt'], d['locality_target_ground_truth'])],
                'mapping': [pt2xym(tokenizer, d['locality_mapping_prompt'], d['locality_mapping_ground_truth'])],
                'struc': [pt2xym(tokenizer, d['locality_struc_prompt'], d['locality_struc_ground_truth'])],
                'tokenSem': [pt2xym(tokenizer, d['locality_tokenSem_prompt'], d['locality_tokenSem_ground_truth'])],
            })
        progress_bar.close() 
    
        print("MedCF data loading complete.", flush=True)
    ...
    # MedCF
    def __medmcqa__(self, tokenizer: AutoTokenizer, data_n, data_path):
        with open(data_path, 'r') as f:
            data = json.load(f)
        ...
        progress_bar = tqdm(data[:sample_count], desc='Medmcqa data preparing...', dynamic_ncols=True)
        for d in progress_bar:
            # Reliability task
            relia_pt.append((d['prompt'], d['target_new']))
            relia_xym.append(pt2xym(tokenizer, d['prompt'], d['target_new']))
            ...
            # Locality task
            loc_pt.append({
                'original': [(d['locality_prompt'], d['locality_ground_truth'])],
            })
            loc_xym.append({
                'original': [pt2xym(tokenizer, d['locality_prompt'], d['locality_ground_truth'])],
            })
        progress_bar.close() 
    
        print("Medmcqa data loading complete.", flush=True)
    
        get_data_by_ids = self.__get_data_by_ids_wrap__(relia_pt, gen_pt, loc_pt, relia_xym, gen_xym, loc_xym)
        return sample_count, get_data_by_ids

    def __medfe__(self, tokenizer: AutoTokenizer, data_n, data_path):
        with open(data_path, 'r') as f:
            data = json.load(f)
        ...
        progress_bar = tqdm(data[:sample_count], desc='MedFE data preparing...', dynamic_ncols=True)
        for d in progress_bar:
            # Reliability task
            relia_pt.append((d['prompt'], d['target_new']))
            relia_xym.append(pt2xym(tokenizer, d['prompt'], d['target_new']))
            ...
            # Locality task
            loc_pt.append({
                'topic': [(d['locality_topic_prompt'], d['locality_topic_ground_truth'])],
                'tokenSem': [(d['locality_tokenSem_prompt'], d['locality_tokenSem_ground_truth'])],
            })
            loc_xym.append({
                'topic': [pt2xym(tokenizer, d['locality_topic_prompt'], d['locality_topic_ground_truth'])],
                'tokenSem': [pt2xym(tokenizer, d['locality_tokenSem_prompt'], d['locality_tokenSem_ground_truth'])],
            })
        progress_bar.close() 
    
        print("MedFE data loading complete.", flush=True)
