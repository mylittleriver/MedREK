...
@dataclass
class MEDREKConfig(EditorConfig):
    @dataclass
...
class MEDREK(BaseEditor):
    def __init__(self, 
        ...
        num_heads=8
        self.prompt_transformer = PromptTransformer(knowledge_rep_dim, 
            model_hidden_size, prompt_token_n, num_heads, self.device)
    ...
    def register_editing_hook(self, begin_layer, lm_head_layer):
        def forward_pre_hook(module, args):
            ...
            if not module.has_past_kv:
                args = args[0]
                args = torch.stack([
                    torch.cat([p.to(inp.device), inp[:-len(p) if len(p) != 0 else None]], 0)
    ...
    def register_model_forward_hook(self, model, model_forward, begin_layer, lm_head):
        if hasattr(model, 'medrek_hooked'):
            return model_forward
        model.medrek_hooked = True
        def forward_medrek(**kargs):
            if 'past_key_values' in kargs and kargs['past_key_values'] != None:
                ...
                post=False
                if (self.auto_retrieve and 'prompt_ids' in kargs and kargs['prompt_ids']!=[]):                 
                    inp_sents = [self.tokenizer.decode(i, skip_special_tokens=True) 
                                 for i in kargs['prompt_ids']]
                    
                elif (self.auto_retrieve and 'prompt_ids' in kargs and kargs['prompt_ids']==[]):
                    inp_sents = [self.tokenizer.decode(i, skip_special_tokens=True) 
                                for i in kargs['input_ids']]

                if self.auto_retrieve and 'prompt_ids' in kargs: # testing stage, otherwise training stage
                    retrieved_ids = self.retrieve_and_get_ids_sim(inp_sents, post)[0]
                ...
                # single gpu
                kargs['input_ids']=kargs['input_ids'].to(self.device)
                kargs['input_ids'] = torch.cat([kargs['input_ids'], pad * self.tokenizer.pad_token_id], 1)
                if self.auto_retrieve and 'prompt_ids' in kargs: 
                    kargs.pop('prompt_ids')
            return model_forward(**kargs)
        return forward_medrek
    ...
    def retrieve_and_get_ids_sim(self, input_queries:List[str], post=False):
        query_reps = self.knowl_rep_model(input_queries, knowl_or_query = 'q') # [n, knowledge_rep_dim] 
        know_key_reps = self.knowledge_base[1:]
    ...
    def name_of_editor_and_model(self)->Tuple[str, str]:
        return 'medrek', self.cfg.edit_model_name
    ...
    def edit_batch(self, requests: List[Dict]):
        ...
        for r in requests:
            if r['prompt'][-1] != ' ' and r['target_new'][0] != ' ':
                ...
                rs_query.append(r['prompt'])
            else:
                rs.append(r['prompt'] + r['target_new'])
                rs_query.append(r['prompt'])  
        self.knowledge_base_nl.extend(rs)
        new_reps_key = self.knowl_rep_model(rs_query, 'q')
        new_reps_val = self.knowl_rep_model(rs, 'k')
        new_prompts = self.prompt_transformer(new_reps_val)
        self.knowledge_base = torch.cat([self.knowledge_base, new_reps_key], 0)
    ...
    def __train_a_batch__(self, contra_knowl, contra_q_rel, contra_q_gen, contra_q_loc,
        ...
        knowledge_reps_key = self.knowl_rep_model(contra_q_rel, 'q') 
        knowledge_reps_val = self.knowl_rep_model(contra_knowl, 'k')

        knowl_rep_prot = self.knowl_rep_model.get_knowl_rep_prot() 
        knowl_reps_with_proto = torch.cat([knowledge_reps_key, knowl_rep_prot])
        ...
        sim_q2 = (q2_reps @ knowledge_reps_key.T) * scale_factor # [bsz, bsz]
        ...
        # edit loss
        self.adopted_prompts = [i for i in self.prompt_transformer(knowledge_reps_val)]
        ...
        print('\nloss_contra:',loss_contra, flush=True)
        print('loss_contra_q1:',loss_contra_q1, flush=True)
        print('loss_contra_q2:',loss_contra_q2, flush=True)
        print('losses_contra_q3:',losses_contra_q3, flush=True)
        print('relia_loss:',relia_loss, flush=True)
        print('gen_losses:',gen_losses, flush=True)
        print('loc_losses:',loc_losses, flush=True)
        log_dict = {
            'Loss': loss,
    ...
    def load_ckpt(self, ckpt_path, restrict = True, load_opt = True):
        ...
        print('Load medrek checkpoints from', ckpt_path)
        return ckpt['loss']
...
def hinge_label_loss(model, input_ids:torch.Tensor, label_ids:torch.Tensor, 
    ...
    #single gpu
    label_ids=label_ids.to(pre_p.device) # [batch, max_len]
    pre_p = pre_p.gather(-1, label_ids.unsqueeze(-1)).squeeze(-1) # [batch, max_len]
    second_pre_p=second_pre_p.to(pre_p.device) 
    masks = masks.to(pre_p.device) 
