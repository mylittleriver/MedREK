...
class PromptTransformer(nn.Module):
    def __init__(self, in_dim=4096, out_dim=4096, prompt_token_n=3, num_heads=8, device = 'cuda:0'):
        super().__init__()
        self.prompt_token_n = prompt_token_n
        ...
        self.attn = nn.MultiheadAttention(embed_dim=out_dim, num_heads=num_heads, batch_first=False)
        self.to(device)

    def forward(self, knowledge_reps: torch.Tensor):
        B = knowledge_reps.size(0)  # batch size
        D = self.out_dim
        T = self.prompt_token_n

        queries = self.query_proj(knowledge_reps).view(B, T, D)  # [B, T, D]

        keys = self.key_proj(knowledge_reps).unsqueeze(1)    # [B, 1, D] -> [B, 1, D] -> [B, T, D]
        values = self.value_proj(knowledge_reps).unsqueeze(1)  # [B, 1, D] -> [B, T, D]

        queries = queries.transpose(0, 1)  # [T, B, D]
        keys = keys.transpose(0, 1)        # [1, B, D]  
        values = values.transpose(0, 1)    # [1, B, D]
